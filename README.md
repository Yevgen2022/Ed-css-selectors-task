# CSS Selectors Mastery Task

A practical CSS project focused on applying various types of selectors to style existing HTML markup without changing its structure. The task reinforces the use of basic, combinator, pseudo-class, and pseudo-element selectors with precision.

## Live Demo
[Click here to view](https://yevgen2022.github.io/Ed-css-selectors-task/)

## Key Concepts Applied

- Used basic selectors to style body, IDs, and class elements
- Applied combinator selectors such as descendant, direct child, and adjacent sibling
- Styled elements contextually based on position or relationship in the DOM
- Used pseudo-classes like `:first-child`, `:last-child`, `:hover`, and `:focus`
- Implemented pseudo-elements like `::before` and `::after` to enhance content semantically
- Ensured precise targeting using only allowed selectors (important for auto-verification)

---

## Technologies

- HTML5
- CSS3
- CSS Selectors (Basic, Combinator, Pseudo-classes, Pseudo-elements)
- GitLab Pages

---

## Task Instructions

See [REQUIREMENTS.md](./REQUIREMENTS.md) for the full original task description and required selectors.
